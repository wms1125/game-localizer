...
class RenPyPipelineTests(unittest.TestCase):
    ...
    def test_catalog_json_is_canonical_and_translation_preserves_placeholders(self):
        ...
        del incomplete["unhandled_items"]
        with self.assertRaisesRegex(ValueError, "fields"):
            RenPyCatalog.from_dict(incomplete)
    ...
    def test_extracts_multiple_literal_translation_calls_from_dynamic_screen_expressions(self):
        ...
        self.assertEqual(
            [
                (item.line, item.expression, item.reason)
                for item in catalog.unhandled_items
            ],
            [
                (4, "_(dynamic_label)", "dynamic_translation_argument"),
                (
                    5,
                    '_("prefix" + suffix)',
        ...
        self.assertEqual(RenPyCatalog.from_dict(catalog.to_dict()), catalog)
        translated = catalog.translate(
            {
                "Page {}": "第 {} 页",
    ...
    def test_writer_rejects_conflicting_targets_for_repeated_source(self):
        temporary, root = self.make_project()
        self.addCleanup(temporary.cleanup)
        (root / "game" / "script.rpy").write_text(
            SCRIPT + '\n    "A narration line."\n', encoding="utf-8"
        )
        catalog = RenPyExtractor().extract(root)
        repeated = [
            item for item in catalog.entries if item.source_text == "A narration line."
        ]
        conflicting = replace(
            catalog,
            entries=tuple(
                replace(
                    ...
                    target_text=(
                        "First target"
                        if item.segment_id == repeated[0].segment_id
                        else "Second target"
                    ),
                )
                if item.source_text == "A narration line."
                else item
        ...
        with self.assertRaisesRegex(
            RenPyValidationError,
            "conflicting translations.*A narration line",
        ) as captured:
            RenPyWriter().build(conflicting, output, source_root=root)

        self.assertIn(repeated[0].segment_id, str(captured.exception))
        self.assertIn(repeated[1].segment_id, str(captured.exception))
    ...
    def test_writer_rejects_runtime_font_probe_without_configured_fonts(self):
        temporary, root = self.make_project()
        self.addCleanup(temporary.cleanup)
        catalog = RenPyExtractor().extract(root).translate(
            {"A narration line.": "甲"}
        ...
        with self.assertRaisesRegex(ValueError, "require configured font_paths"):
            RenPyWriter().build(
                catalog,
                root / "localized-output",
                source_root=root,
    ...
    def test_writer_rejects_runtime_probe_codepoint_missing_from_fonts(self):
        temporary, root = self.make_project()
        self.addCleanup(temporary.cleanup)
        font = root / "primary.ttf"
        font.write_bytes(b"primary font")
        catalog = RenPyExtractor().extract(root).translate(
            {"A narration line.": "甲"}
        )
        ...
        with patch(
            "game_localizer.hanengine.renpy._font_unicode_coverage",
            return_value={ord("甲")},
        ), self.assertRaisesRegex(ValueError, "U\\+2605"):
            RenPyWriter().build(
                catalog,
                root / "localized-output",
                source_root=root,
    ...
    def test_writer_assigns_runtime_probe_to_covering_fallback_font(self):
        temporary, root = self.make_project()
        self.addCleanup(temporary.cleanup)
        ...
        config_text = (
            root / "localized-output" / "game" / "hanengine_fonts.rpy"
        ).read_text(encoding="utf-8")
        self.assertIn(
            '_hanengine_font_group.add("hanengine_fonts/primary.ttf", 0x7532, 0x7532)',
            config_text,
        )
        self.assertIn(
            '_hanengine_font_group.add("hanengine_fonts/fallback.ttf", 0x2605, 0x2605)',
            config_text,
        )
        self.assertIn(
            '_hanengine_font_group.add("hanengine_fonts/fallback.ttf", None, None)',
            config_text,
        )
