...
状态：历史 MVP 规格，已由多引擎 AdapterV1 实施方向取代

> 范围更新（2026-08-08）：HanEngine 的当前目标是统一支持 Ren'Py、RPG Maker MV/MZ、Godot、Unity 与 Unreal 等主流引擎。本文中将 Ren'Py 写为唯一工作室引擎的段落，描述的是早期 MVP 的拆分，不得用于限制后续实现或产品文案。当前支持状态以 `README.md` 和 `docs/project-status.md` 为准。
...
2. 专业模式：对 AdapterV1 已就绪的多引擎明文项目完成检测、提取、翻译、校验、独立构建、验证、补丁导出和回滚。

两种模式共享统一文本段、翻译记忆、术语表、模型路由、任务系统和本地存储。HanEngine 首先作为可独立运行的无 GUI 引擎达到发布门槛，Electron + React 桌面端是其薄客户端。
...
- 普通模式：单个已授权游戏的快速汉化入口、风险检测、独立输出和任务中心。
- 专业模式：多引擎项目入口、手动提取/目录编辑/构建、封包流程与实时任务中心。
...
- TTS、语音克隆和自动配音。
- 未由对应 AdapterV1 明确声明为 `build_ready` 的资源形态、版本组合或引擎专用二进制格式的写回承诺。所有引擎都必须通过能力门控和 HanGuard 最终路由。
- 对加密或专有封装资源的自动解包。

### 3.3 交付拆分

本文保留早期 MVP 的设计背景，不对应当前单一实现提交。当前实现以多引擎 AdapterV1、HanPipelineV1、HanTask Center 和 Electron + React 为基础；后续扩展必须在统一契约、能力门控和真实授权项目验证之上进行。

## 4. 用户体验

### 4.1 双模式外壳

- `普通` 为默认模式，对应后端 `player`。
- `专业` 为高级模式，对应后端 `studio`。
...
用户打开已授权的多引擎项目后，HanEngine 自动完成检测，并依据适配器资源范围与 HanGuard 决策选择检测、提取、翻译、自动校验、独立构建和验证。异常条目进入异常清单，不要求用户逐条审核全部文本；无法安全处理的项目停留在仅检测状态。

## 5. 系统架构

### 5.1 HanShell

Electron + React 双模式桌面外壳。只负责输入、视图、任务控制、日志和状态呈现。耗时工作由 Electron 主进程启动 Python 子进程，React 不直接访问 Node 或项目文件系统。
...
当前专业模式使用 AdapterV1 注册表统一承载 RPG Maker MV/MZ、Godot、Unity 和 Unreal 的结构化资源适配；Ren'Py 保留独立明文适配，后续纳入同一运行时。玩家侧能力为屏幕捕获、OCR、区域跟踪、背景遮盖和同位置文字重绘，但只在 HanGuard 允许的安全边界内运行。
...
- 合法多引擎示例项目的检测、提取、翻译、独立构建、验证和回滚端到端测试。
...
- 每个已声明 `build_ready` 的引擎资源范围内，文本提取覆盖率、占位符保持率和独立构建验证都必须满足其对应基准。
...
1. 建立适配器契约测试骨架、HanGuard 风险样本、覆盖主流引擎的基准项目清单和玩家截图生成清单。
2. 建立无 GUI 的 HanCore、Segment、RoutePlan、HanTask 和 HanStore。
3. 迁移现有明文翻译、编码处理、扫描器和检测器，保持现有测试通过。
4. 逐个调教 Ren'Py、RPG Maker MV/MZ、Godot、Unity 和 Unreal 适配器；只有通过各自真实授权项目验证的资源范围才能达到“已验证可用”。
5. 生成固定截图/动态序列基准，建立视觉替换管线，调教 OCR、区域跟踪、遮盖和排版。
6. 接入翻译记忆、术语表、机器翻译和 LLM 分层路由。
7. 接入 Electron + React 双模式外壳和实时任务中心。
...
- 专业模式能够对每个已验证的主流引擎基准项目完成可回滚的原生资源汉化闭环。
...
- 第 11 节明确了多引擎/截图基准集、参考硬件和全部量化指标。
